...
class PedidoDeUniformeScreen(Screen):
    ...
    def mostrar_uniforme(self):
        ...
        if not all([item_solicitado, tamanho, posto, quantidade]):
            ...
            buttons = BoxLayout(orientation='horizontal', spacing=10, size_hint=(1, 0.3))
            btn_confirmar = Button(text="Confirmar", on_release=lambda x: self.confirmar_solicitacao(
                nome_vigilante, matricula, posto, item_solicitado, tamanho, quantidade))
            btn_cancelar = Button(text="Cancelar", on_release=self.fechar_popup)
            buttons.add_widget(btn_confirmar)
            buttons.add_widget(btn_cancelar)
            content.add_widget(buttons)
    ...
    def confirmar_solicitacao(self, nome_vigilante, matricula, posto, item_solicitado, tamanho, quantidade):
        self.fechar_popup()
        payload = {
            ...
            "quantidade": quantidade}
        try:
            resposta = requests.post(f'{API_URL}/api/pedido_uniforme', json=payload)
            if resposta.status_code == 200:
                show_popup('Sucesso', 'Solicitação enviada com sucesso!')
                ...
                show_popup('Erro', f'Falha ao enviar: {erro}')
        except Exception as e:
            show_popup('Erro', f'Erro de conexão: {str(e)}')
